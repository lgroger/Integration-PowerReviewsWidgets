...
*/					        	var arrayData = Ext.getCmp('menuData');
					        ...
					        handler: function() {
					           Ext.getCmp("addMenuWin").destroy();
					        	...
					        	width:"250",
					        	handler:function(){
					        		Ext.getCmp("gridviewlv1").getStore().add({ 'mcap':'Menu Caption','mlink': '','ntab':false});
									//alert(Ext.getCmp("gridview").getStore().getCount());
										...
										store: Ext.create("testAppimg.store.Objects",{
							        		...
							        		listeners:{
							        			add:function( store, records, index, opts ){
							        				...
							        				arrtmp.push({"mcap":records[0].data.mcap,"mlink":records[0].data.mlink,"ntab":records[0].data.ntab,"mimg":records[0].data.mimg,"items":[]});
				            ...
				            items: [
				                {
				                    boxLabel  : 'Right',
				                    name      : 'size',
				                    inputValue: 'left-align',
				                    id        : 'leftali',
				                    margin    : '0 10px 0 0'
				                }, {
				                    boxLabel  : 'Bottom',
				                    name      : 'size',
				                    inputValue: 'bottom-align',
				                    id        : 'bottomali'
				                }
				            ]
				        },{
				        	 ...
				        	 step:5
								...
								handler:function(){
									if(Ext.isDefined(Ext.getCmp("promoImg").getValue()) && Ext.getCmp("promoImg").getValue().length >0 && Ext.getCmp("imagebtn").items.get(0).getGroupValue() !== null){
										//alert(rowIndex);
										//alert(Ext.getCmp("promoImg").getValue());
										var arrtmp=Ext.getCmp("menuDatatmp").getValue();
										arrtmp[rowIndex].banner={"imgUrl":Ext.getCmp("promoImg").getValue(),"url":Ext.getCmp("plink").getValue(),"imgAlign":Ext.getCmp("imagebtn").items.get(0).getGroupValue(),"imgWidth":Ext.getCmp("imWidth").getValue()};
										Ext.getCmp("menuDatatmp").setValue(arrtmp);
										Ext.getCmp("promoimgselect").destroy();
									}else{
										Ext.Msg.alert("Error","Please Fill all the fileds.");
									}
								}
							},{
								xtype:"button",
								...
								afterrender:function(){
									...
									if(Ext.isDefined(arrtmp[rowIndex].banner)){
										...
										if(Ext.isDefined(arrtmp[rowIndex].banner.imgAlign)){
											if(arrtmp[rowIndex].banner.imgAlign==="bottom-align"){
												Ext.getCmp("bottomali").setValue(true);
											}else{
												Ext.getCmp("leftali").setValue(true);
											}
										}
										if(Ext.isDefined(arrtmp[rowIndex].banner.imgWidth)){
											Ext.getCmp("imWidth").setValue(arrtmp[rowIndex].banner.imgWidth);
										}
									}else{
										Ext.getCmp("bottomali").setValue(true);
									}
						...
						buttons:[
	               		{
					        ...
					        handler: function() {
					           Ext.getCmp("addMenuWinlv3").destroy();
					        	...
					        	width:"250",
					        	handler:function(){
					        		Ext.getCmp("gridviewlv3").getStore().add({ 'mcap':'Menu Caption','mlink': '','ntab':false});
									//alert(Ext.getCmp("gridview").getStore().getCount());
									//console.log("addddedd");
									...
									rowEditinglv3.startEdit((Ext.getCmp("gridviewlv3").getStore().getCount()-1), 0);
										...
										store: Ext.create("testApp.store.Objects",{
							        		...
							        		listeners:{
							        			add:function( store, records, index, opts ){
							        				var arrtmp=Ext.getCmp("menuDatatmp").getValue();
							        				arrtmp.push({"mcap":records[0].data.mcap,"mlink":records[0].data.mlink,"ntab":records[0].data.ntab,"items":[]});
							        				Ext.getCmp("menuDatatmp").setValue(arrtmp);
									    ...
									    columns : [{
									        ...
									        items: [{
									          icon: '/resources/images/add-menu.png',
									            ...
									            margin:"10",
									            handler: function(grid, rowIndex, colIndex) {
									            	if(Ext.getCmp("addMenuWinlv3").flag){
										                Ext.MessageBox.show({
														    title:'Menu Editor',
														    msg: 'Changes are not saved are you sure you want to continue?',
														    buttonText: {yes: "Save and Continue",no: "Discard",cancel: "Cancel"},
														    fn: function(btn){
														    	if(btn=="yes"){
														        	...
														        	arr[Ext.getCmp("addMenuWinlv3").topidx].items[Ext.getCmp("addMenuWinlv3").idx].items=Ext.getCmp("menuDatatmp").getValue();
														        	arrayData.setValue(arr);
														    		var sids=Ext.getCmp("addMenuWinlv3").topidx;
														    		var pids=Ext.getCmp("addMenuWinlv3").idx;
																	Ext.getCmp("addMenuWinlv3").destroy();
																	//Ext.getCmp("menuWidget").showEditChildlv3(ids,rowIndex);
																	Ext.getCmp("menuWidget").showEditChildlv4(sids,pids,rowIndex);
														    	}else if(btn=="no"){
																	var pids=Ext.getCmp("addMenuWinlv3").idx;
																	Ext.getCmp("addMenuWinlv3").destroy();
																	//Ext.getCmp("menuWidget").showEditChildlv3(ids,rowIndex);
									            	...
									            	//Ext.getCmp("menuWidget").showEditChildlv4(Ext.getCmp("addMenuWinlv3").topidx,Ext.getCmp("addMenuWinlv3").idx,rowIndex);
									                //alert("Edit " +rowIndex);
									                //Ext.getCmp("menuWidget").showEditChildlv3(Ext.getCmp("addMenuWin").idx,rowIndex);
									            }
									        },{
									            icon: '/resources/images/del-icon.png',
									            tooltip: 'Delete',
	...
	},showEditChildlv4:function(supertopidx,topidx,paridx){
		console.log(supertopidx+" - "+topidx+" - "+paridx);
			Ext.create('menuWin',{
			title:"Create/Edit Child for "+Ext.getCmp("menuData").getValue()[supertopidx].mcap+" -> "+Ext.getCmp("menuData").getValue()[supertopidx].items[topidx].mcap+" -> "+Ext.getCmp("menuData").getValue()[supertopidx].items[topidx].items[paridx].mcap,
			id:"addMenuWinlv4",
			supertopidx:supertopidx,
			topidx:topidx,
			paridx:paridx,
			items:[{
				xtype:"menuform",
				...
				buttons:[
           		{
			        ...
			        handler: function(){
			        	...
			        	/*var arrayData = Ext.getCmp('menuData');
						var arr = arrayData.getValue();
			        	arr[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].items=[];
			        	var records = Ext.getCmp('gridviewlv4').getStore().getRange();
			        	 for (var i = 0; i < records.length; i++) {
			        	 	console.log(records[i].data);
			        	 	arr[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].items.push(records[i].data);
			        	 }
			        	 arrayData.setValue(arr);*/
						var arrayData = Ext.getCmp('menuData');
						var arr = arrayData.getValue();
						arr[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].items=Ext.getCmp("menuDatatmp").getValue();
						arrayData.setValue(arr);
			        	//Ext.getCmp("menuData").getValue()[Ext.getCmp("addMenuWin").idx].items.push(Ext.getCmp('gridviewlv1').getStore().getRange());
			        	Ext.getCmp("addMenuWinlv4").destroy();
			        }
           		},{
			        text: "<div style='color:#000;width:150px;padding:10px'>Close</div>",
			        margin:"5",
			        ...
			        handler: function() {
			           Ext.getCmp("addMenuWinlv4").destroy();
			        	...
			        	width:"250",
			        	handler:function(){
			        		Ext.getCmp("gridviewlv4").getStore().add({ 'mcap':'Menu Caption','mlink': '','ntab':false});
							//alert(Ext.getCmp("gridview").getStore().getCount());
							//console.log("addddedd");
							rowEditinglv4.cancelEdit();
							rowEditinglv4.startEdit((Ext.getCmp("gridviewlv4").getStore().getCount()-1), 0);
			        	}
			        },{
			        	xtype:"button",
			        	margin:"10",
			        	text:"<div style='color:#000;'>No of items in columns</div>",
			        	width:"250",
			        	handler:function(){
						var arrayData = Ext.getCmp('menuData');
						var arr = arrayData.getValue();
							Ext.Msg.prompt("Add New","Please Enter No of items per columns",function(btn,newNo){
								if(newNo!="" && btn=="ok"){
									//alert(newNo);
						arr[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].rcount=newNo;
						arrayData.setValue(arr);
						//Ext.Msg.alert("Updated");
								//arr[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx]
								}
							},this,false,arr[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].rcount);
			        	}
			        },{			xtype:"gridNav",
								id:"gridviewlv4",
								title:"",
								...
								store: Ext.create("testApp.store.Objects",{
									...
									listeners:{
										add:function( store, records, index, opts ){
					        				var arrtmp=Ext.getCmp("menuDatatmp").getValue();
					        				arrtmp.push({"mcap":records[0].data.mcap,"mlink":records[0].data.mlink,"ntab":records[0].data.ntab});
					        				Ext.getCmp("menuDatatmp").setValue(arrtmp);
					        				//Ext.getCmp("addMenuWinlv3").flag=true;
					        			},
						        		update:function( store, record, operation, eOpts ){
											var arrtmp = Ext.getCmp('menuDatatmp').getValue();
											console.log("Update arrtp");
											console.log(arrtmp);
											arrtmp[store.indexOf(record)].mcap=record.data.mcap;
											arrtmp[store.indexOf(record)].mlink=record.data.mlink;
											arrtmp[store.indexOf(record)].ntab=record.data.ntab;
							    ...
							    columns : [{
							        ...
							        items: [{
							            ...
							            handler: function(grid, rowIndex, colIndex) {
							                  Ext.Msg.confirm("Menu Editor","Are you Sure you want to delete this menu and child items?",function(btn){
							                 	if(btn=="yes"){
							                 		Ext.getCmp("gridviewlv4").getStore().removeAt(rowIndex);									                 		
							                 	}
							                 });
							            }
							        },{
									            ...
									            handler: function(grid, rowIndex, colIndex) {
									                 //alert("Edit " +rowIndex);
									                 Ext.getCmp("menuWidget").moveDownrow(rowIndex,"gridviewlv4");
									                 console.log("ridx "+rowIndex);
									            }
									        },{
									            icon: '/resources/images/icon-up.png',
									            ...
									            handler: function(grid, rowIndex, colIndex) {
									                 //alert("Edit " +rowIndex);
									                 Ext.getCmp("menuWidget").moveUprow(rowIndex,"gridviewlv4");
									                 console.log("ridx "+rowIndex);
									            }
									        }]
							    }],listeners : {
									    afterrender:function(){
									    	console.log("loading lv4");
									    	console.log(Ext.getCmp("menuData").getValue()[Ext.getCmp("addMenuWinlv4").supertopidx]);
									    	if (this.store.getCount() == 0) {
								            console.log('store not loaded yet');
								           this.store.loadData(Ext.getCmp("menuData").getValue()[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].items);
								           Ext.getCmp("menuDatatmp").setValue(Ext.getCmp("menuData").getValue()[Ext.getCmp("addMenuWinlv4").supertopidx].items[Ext.getCmp("addMenuWinlv4").topidx].items[Ext.getCmp("addMenuWinlv4").paridx].items);
								            this.store.on('load', function() {
								                console.log('load after render');
								            }, this, {
								                single: true
