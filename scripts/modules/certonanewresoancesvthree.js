require(['shim!//edge1.certona.net/cd/67195178/shindigz.com/scripts/resonance[modules/jquery-mozu=jQuery]>jQuery', 'underscore', 'modules/api','modules/jquery-mozu'], function(certonaLib,_, api,$) {
      ...
      _.each(recomm,function(v,i){	
        ...
        if(window.recommendedproducts.resonance.schemes.length > 1){
	         if(_.findWhere(window.recommendedproducts.resonance.schemes[0].items,{id:v.productCode})!== undefined){
	         	 ...
	         	 prod_obj=_.findWhere(window.recommendedproducts.resonance.schemes[1].items,{id:v.productCode});
			if(prod_obj!==undefined){
		         		if(prod_obj.Personalized==="True"){
		         			isPersionalize=true;
		         		}else{
        ...
        htmltemp += '<a href="/'+recomm[i].content.seoFriendlyUrl+'/p/'+recomm[i].productCode;
        if(recs){
