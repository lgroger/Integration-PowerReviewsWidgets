...
var certonaRecommendations = function(response){
    ...
    var htmlfun = function(recomm,divid,personalizeflag){
      ...
      $('.echi-shi-related-products-slider .owl-carousel').owlCarousel({
		...
		var recommendedProductSearch = function(filter_q,flag_cart,recomm){
			...
			api.request('GET',api_search_query).then(function(res){
				...
				var personalize = [];
				var personalizepdp = [];
				for(var p =0;p<recomm.resonance.schemes.length;p++){
				for(var q=0;q<recomm.resonance.schemes[p].items.length;q++){
				if(recomm.resonance.schemes[p].scheme === 'product1_rr'){
				personalize.push(recomm.resonance.schemes[p].items[q].Personalized);
				}
				else if(recomm.resonance.schemes[p].scheme === 'product2_rr'){
				personalizepdp.push(recomm.resonance.schemes[p].items[q].Personalized);
				}
				else{
				personalize.push(recomm.resonance.schemes[p].items[q].Personalized);
				}
                    ...
                    var products;
					 ...
					 //for (var i = 0; i < recomm.resonance.schemes.length; i++){
					 	_.each(recomm.resonance.schemes,function(v,i){
								...
								htmlfun(response.items,addtoc,personalize);
								}
								else if(recomm.resonance.schemes[i].scheme === "category1_rr"){
									//var catprod = response.items;
									htmlfun(response.items,cat1_rr,personalize); 
								}
								...
								htmlfun(products,prd1_rcc,personalize);
								...
								htmlfun(product_2,prd2_rcc,personalizepdp);
